...
// MARK: - UITableView
public class SmartTableView: UITableView, UITextFieldDelegate, UITextViewDelegate {
...
private extension SmartTableView {
    func setup() {
        guard !hasAutomaticKeyboardAvoidingBehaviour else { return }
...
public class SmartCollectionView: UICollectionView, UITextViewDelegate {
    override public var contentSize: CGSize {
        willSet(newValue) {
            guard !newValue.equalTo(self.contentSize) else { return }
...
private extension SmartCollectionView {
    func setup() {
        NotificationCenter.default.addObserver(self, selector: #selector(TPKeyboardAvoiding_keyboardWillShow(_:)), name: UIResponder.keyboardWillChangeFrameNotification, object: nil)
...
// MARK: - UIScrollView
public class SmartScrollView: UIScrollView, UITextFieldDelegate, UITextViewDelegate {
    override public var contentSize: CGSize {
        didSet {
...
private extension SmartScrollView {
    func setup() {
        NotificationCenter.default.addObserver(self, selector: #selector(TPKeyboardAvoiding_keyboardWillShow(_:)), name: UIResponder.keyboardWillChangeFrameNotification, object: nil)
...
extension UIScrollView {
    @objc func TPKeyboardAvoiding_keyboardWillShow(_ notification: Notification) {
        ...
        if !state.keyboardVisible {
            ...
            state.verScrollIndicatorInsets = self.verticalScrollIndicatorInsets
            state.horScrollIndicatorInsets = self.horizontalScrollIndicatorInsets
    ...
    @objc func TPKeyboardAvoiding_keyboardWillHide(_ notification: Notification) {
        ...
        UIView.animate(withDuration: TimeInterval(duration), delay: 0, options: options, animations: { [weak self] in
            guard let self = self, self is SmartScrollView else { return }
            
            self.contentSize = state.priorContentSize
            self.contentInset = state.priorInset
            self.verticalScrollIndicatorInsets = state.verScrollIndicatorInsets
            self.horizontalScrollIndicatorInsets = state.horScrollIndicatorInsets
...
internal class TPKeyboardAvoidingState: NSObject {
    ...
    var verScrollIndicatorInsets = UIEdgeInsets.zero
    var horScrollIndicatorInsets = UIEdgeInsets.zero
